...
public class MyFileSystem implements FileSystem {
    ...
    private DirectBlock getDirectBlock(int fd, MODE mode) {//FIXME!!!
        ...
        return new DirectBlock(disk, inode.ptr[blockNum], blockOff, fresh);
        ...
        else if(blockNum<10+IndirectBlock.COUNT){//Single indirect
        	...
        	disk.read(inode.ptr[10], ib);
        	boolean fresh=ib.ptr[blockNum-10]==0;
        	if(fresh){
        		if(mode == MODE.r){                   
        			return DirectBlock.hole;
        		}
                else{
                	ib.ptr[blockNum-10] = freeMap.find();
                	if(ib.ptr[blockNum-10]==0){
                		return null;
                	}
                }
                    
        	}
        	disk.write(inode.ptr[10], ib);
        	return new DirectBlock(disk, ib.ptr[blockNum-10], blockOff, fresh);
            	
            	
            }else if(blockNum<10+IndirectBlock.COUNT+(IndirectBlock.COUNT*IndirectBlock.COUNT)){//double indirect
            	...
            	if(inode.ptr[11]==0){        		
            		if(mode == MODE.r){
            			...
            			IndirectBlock ib= new IndirectBlock();
            			for(int i=0; i<IndirectBlock.COUNT; i++){
            				if((ib.ptr[i]=freeMap.find())==0)
            					return null;
            				disk.write(ib.ptr[i], new IndirectBlock());
            			}
            			disk.write(inode.ptr[11], ib);
            			
            		}                               		         		
                	}
            	
            	IndirectBlock ib1=new IndirectBlock();
            	disk.read(inode.ptr[11], ib1);
            	IndirectBlock ib2=new IndirectBlock();
            	disk.read(ib1.ptr[(blockNum-10-IndirectBlock.COUNT)/IndirectBlock.COUNT], ib2);
            	boolean fresh=ib2.ptr[(blockNum-10-IndirectBlock.COUNT)%IndirectBlock.COUNT]==0;
            	if(fresh){
            		if(mode == MODE.r){                   
            			return DirectBlock.hole;
            		}
                    else{
                    	ib2.ptr[(blockNum-10-IndirectBlock.COUNT)%IndirectBlock.COUNT] = freeMap.find();
                    	if(ib2.ptr[(blockNum-10-IndirectBlock.COUNT)%IndirectBlock.COUNT]==0){
                    		return null;
                    	}
                    }
                        
            	}
            	disk.write(ib1.ptr[(blockNum-10-IndirectBlock.COUNT)/IndirectBlock.COUNT], ib2);
            	return new DirectBlock(disk, ib2.ptr[(blockNum-10-IndirectBlock.COUNT)%IndirectBlock.COUNT], blockOff, fresh);
            	...
            	if(inode.ptr[12]==0){        		
            		if(mode == MODE.r){
            			...
            			disk.write(inode.ptr[12], ib);
            			
            		}      		
                	}
            	IndirectBlock ib1=new IndirectBlock();
            	disk.read(inode.ptr[12], ib1);
            	IndirectBlock ib2=new IndirectBlock();
            	disk.read(ib1.ptr[(blockNum-10-IndirectBlock.COUNT-(IndirectBlock.COUNT*IndirectBlock.COUNT))/IndirectBlock.COUNT/IndirectBlock.COUNT], ib2);
            	IndirectBlock ib3= new IndirectBlock();
            	disk.read(ib2.ptr[(blockNum-10-IndirectBlock.COUNT-(IndirectBlock.COUNT*IndirectBlock.COUNT))/IndirectBlock.COUNT], ib3);
            	boolean fresh=ib3.ptr[(blockNum-10-IndirectBlock.COUNT-(IndirectBlock.COUNT*IndirectBlock.COUNT))%IndirectBlock.COUNT]==0;
            	if(fresh){
            		if(mode == MODE.r){                   
            			return DirectBlock.hole;
            		}
                    else{
                    	ib3.ptr[(blockNum-10-IndirectBlock.COUNT-(IndirectBlock.COUNT*IndirectBlock.COUNT))%IndirectBlock.COUNT] = freeMap.find();
                    	if(ib3.ptr[(blockNum-10-IndirectBlock.COUNT-(IndirectBlock.COUNT*IndirectBlock.COUNT))%IndirectBlock.COUNT]==0){
                    		return null;
                    	}
                    }
                        
            	}
            	disk.write(ib2.ptr[(blockNum-10-IndirectBlock.COUNT-(IndirectBlock.COUNT*IndirectBlock.COUNT))/IndirectBlock.COUNT], ib3);
            	disk.write(ib1.ptr[(blockNum-10-IndirectBlock.COUNT-(IndirectBlock.COUNT*IndirectBlock.COUNT))/IndirectBlock.COUNT/IndirectBlock.COUNT], ib2);
            	return new DirectBlock(disk, ib3.ptr[(blockNum-10-IndirectBlock.COUNT-(IndirectBlock.COUNT*IndirectBlock.COUNT))%IndirectBlock.COUNT], blockOff, fresh);
